...
def test_is_type_validator():
    ...
    for value, value_type in example_properties:
        ...
        assert validate(
            value
        ), f"is_type validator failed for type {value_type}, value {value}"
...
def test_list_of_validator(value, item_type, should_pass):
    ...
    if should_pass:
        assert validate(
            value
        ), f"list_of validator failed for item type {item_type}, value {value}"
...
def test_one_of_validator(value, validators, should_pass):
    ...
    if should_pass:
        assert validate(
            value
        ), f"one_of validator failed for validators {validators}, value {value}"
