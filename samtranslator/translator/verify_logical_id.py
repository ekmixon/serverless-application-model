...
def verify_unique_logical_id(resource, existing_resources):
    ...
    return (
        resource.logical_id is None
        or resource.logical_id not in existing_resources
        or resource.resource_type in do_not_verify
        and existing_resources[resource.logical_id]["Type"]
        in do_not_verify[resource.resource_type]
    )
