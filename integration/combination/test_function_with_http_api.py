...
class TestFunctionWithHttpApi(BaseTest):
    def test_function_with_http_api(self):
        ...
        self.verify_get_request_response(f"{base_url}some/path", 200)
        self.verify_get_request_response(f"{base_url}something", 404)
        self.verify_get_request_response(f"{base_url}another/endpoint", 404)
